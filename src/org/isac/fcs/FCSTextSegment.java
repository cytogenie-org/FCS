...
public class FCSTextSegment
{
      ...
      throws FCSException
  {
    if (keyword.length() > FCS.MAX_KEYWORD)
      throw new FCSException("Keyword is too long " + keyword);
    if (value.length() > FCS.MAX_VALUE) 
    	throw new FCSException("Value is too long " + value.length() + " for the keyword: " + keyword);
    ...
    if (value.contains("Event_length") || value.contains("FileNum")) {
    	System.out.println("Break here");
    	return;
    }
  //  System.out.println("Setting: " + keyword + " value: " + value);
      ...
      throws FCSException
  {
    ...
    while (pos < len)
    {
      ...
      if (keyword.length() == 0 || value.length() == 0) {
    	 /* if (keyword.length() != 0 && "CHARSET".equals(keyword.toString().trim())) {
    		  System.out.println("WARNING: Keyword CHARSET is present but its value is malformed" );
    	  }
    	  else if (keyword.length() != 0 && "GROUPNAME".equals(keyword.toString().trim())) {
    		  System.out.println("WARNING: Keyword GROUPNAME is present but its value is malformed" );
    	  }*/
    	  if (keyword.length() != 0) {
    		  System.out.println("WARNING: Malformed FCS text segment for keyword " + keyword);
    	  }
    	  else {
